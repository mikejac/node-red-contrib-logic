            ...
            for (let i = 0; i < this.data.length; i++) {
                ...
                if (msg) { // if there is a msg
                    ...
                    if ((rule.propertyType === 'msg' && msg.topic === rule.topic) || rule.propertyType !== 'msg') { // if the rule match with the message got
                        if (typeof test == "undefined") {
                           // console.log('typeof test == "undefined"')
                            reject('test is undefined')
                        }
                        //console.log('test =', test)
                        //console.log('v1 = ', v1)
                        //console.log('v2 = ', v2)
                        //console.log('rule.case = ', rule.case)

                        if (operators[rule.t](test, v1, v2, rule.case)) {
                            //console.log('** msg; rule.validated = true **')
                            rule.validated = true;
                        } else {
                            //console.log('** msg; rule.validated = false **')
                    ...
                    if (rule.propertyType !== 'msg') { // if the rule isn't about a message
                        if (operators[rule.t](test, v1, v2, rule.case)) {
                            //console.log('** no msg; rule.validated = true **')
                            rule.validated = true;
                        } else {
                            //console.log('** no msg; rule.validated = false **')
                            rule.validated = false
                        }
                        rule.previousValue = test;
                    }
